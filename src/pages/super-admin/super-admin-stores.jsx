...
import { deletSingleStoreFn, getAllStoresFn, updatePaidStoreStatusFn, updateStoreStatusFn } from "@/services/super-admin/store-service";
...
export default function SuperAdminStoreList() {
    ...
    const updatePaidStatus = (store_id) => {
        updatePaidStoreStatusFn(store_id).then(() => {
            getData();
        })
    }
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableCell className="text-sm text-center font-bold">Order ID</TableCell>
                        <TableCell className="text-sm text-center font-bold">Status</TableCell>
                ...
                {loading && <ShimmerTableBody row={10} coloumn={15} />}
                {!loading &&
                    <TableBody>
                        {!loading && stores.length === 0 && (<TableRow>
                            <TableCell className="min-h-[500px]" colSpan={15}>
                        ...
                        {stores?.map(store => (
                            <TableRow key={store.store_id}>
                                ...
                                <TableCell className="text-sm text-center">{store.order_id}</TableCell>
                                <TableCell className="text-sm text-center">
                                    {store.paid_status === "PAID" && store.paid_status}
                                    {store.paid_status === "UNPAID" && <Switch
                                        checked={store.paid_status !== "UNPAID"}
                                        onCheckedChange={() => updatePaidStatus(store.store_id)}
                                    />}
                                </TableCell>
